...
export function TopRatedAnimes() {
  const [isPlaying, setIsPlaying] = useState(0);
  const { topRatedAnimes } = useContext(TopRatedContext);
  ...
  return (
    <Container>
      <h2>
        Top Rated Animes
        <span title="See more top rated animes">
          <Link href="/top-rated">
            <a>see more</a>
          </Link>
        </span>
      ...
      {slideCounter > 0 && (
        <SlideCarouselButton
          onClick={() =>
            handleCarouselButton('previous', topRatedAnimes.length)
          }
          type="button"
      ...
      {slideCounter < slideLimit && (
        <SlideCarouselButton
          onClick={() => handleCarouselButton('next', topRatedAnimes.length)}
          type="button"
          buttonHeight={playerHeight}
        >
